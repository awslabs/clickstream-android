...
class TestLogcat:
    ...
    def test_launch_events(self, path):
        ...
        assert self.recorded_events[0]['event_name'] == '_first_open'
        assert self.recorded_events[1]['event_name'] == '_app_start'
        assert self.recorded_events[2]['event_name'] == '_session_start'
        print("Verifying successful order of launch events.")

    @pytest.mark.parametrize("path", path)
    def test_screen_view(self, path):
        print("Start verify: " + str(path))
        self.init_events(path)
        # assert _screen_view
        assert self.recorded_events[3]['event_name'] == '_screen_view'
    ...
    @pytest.mark.parametrize("path", path)
    def test_view_wishlist(self, path):
        print("Start verify: " + str(path))
        self.init_events(path)
    ...
    @pytest.mark.parametrize("path", path)
    def test_view_account(self, path):
        print("Start verify: " + str(path))
        self.init_events(path)
        # assert view_account
